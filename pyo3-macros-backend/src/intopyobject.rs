...
impl<'a, const REF: bool> Container<'a, REF> {
    ...
    fn output_type(&self) -> PythonTypeHint {
        match &self.ty {
            ...
            ContainerType::Tuple(tups) => PythonTypeHint::subscript(
                PythonIdentifier::builtin("tuple"),
                tups.iter().map(
                    |TupleStructField {
                         into_py_with,
                         field,
            ...
            ContainerType::Struct(_) => {
                ...
                PythonIdentifier::module_attr("_typeshed", "Incomplete").into()
            }
        }
    }
    ...
    fn field_output_type(
        ...
        if into_py_with.is_some() {
            ...
            PythonIdentifier::module_attr("_typeshed", "Incomplete")
        } else {
            PythonIdentifier::from_into_py_object(ty.clone(), None)
        }
        .into()
    }
}
